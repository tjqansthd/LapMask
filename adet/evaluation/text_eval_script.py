...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for k in gt:
        rrc_evaluation_funcs.validate_lines_in_file_gt(k, gt[k], evaluationParams['CRLF'], evaluationParams['LTRB'],
                                                       True)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    def transcription_match(transGt, transDet, specialCharacters=str(r'!?.:,*"()·[]/\''),
    ...
    def include_in_dictionary(transcription):
        ...
        range3 = [ord(u'?'), ord(u'ƿ')]
        range4 = [ord(u'?'), ord(u'ɿ')]
        range5 = [ord(u'?'), ord(u'Ͽ')]
        range6 = [ord(u'-'), ord(u'-')]
    ...
    for resFile in gt:
        ...
        for n in range(len(pointsList)):
            ...
            det_only_dontCare = dontCare = transcription == "###"  # ctw1500 and total_text gt have been modified to
            # the same format.
        ...
        if resFile in subm:
            ...
            pointsList, confidencesList, transcriptionsList = rrc_evaluation_funcs. \
                ...
                for gtNum in range(len(gtPols)):
                    for detNum in range(len(detPols)):
                        if det_only_gtRectMat[gtNum] == 0 and det_only_detRectMat[
                            detNum] == 0 and gtNum not in det_only_gtDontCarePolsNum and detNum not in \
                                det_only_detDontCarePolsNum:
                            if iouMat[gtNum, detNum] > evaluationParams['IOU_CONSTRAINT']:
        ...
        det_only_hmean = 0 if (
                                      det_only_precision + det_only_recall) == 0 else 2.0 * det_only_precision * \
                                                                                      det_only_recall / (
                                                                                              det_only_precision +
                                                                                              det_only_recall)
    ...
    det_only_methodHmean = 0 if det_only_methodRecall + det_only_methodPrecision == 0 else 2 * det_only_methodRecall \
                                                                     * det_only_methodPrecision / (
                                                                    det_only_methodRecall + det_only_methodPrecision)

    methodMetrics = r"E2E_RESULTS: precision: {}, recall: {}, hmean: {}".format(methodPrecision, methodRecall,
                                                                                methodHmean)
    ...
    resDict = {'calculated': True, 'Message': '', 'e2e_method': methodMetrics,
               'det_only_method': det_only_methodMetrics, 'per_sample': perSampleMetrics}
...
def text_eval_main(det_file, gt_file, is_word_spotting):
    ...
    return rrc_evaluation_funcs.main_evaluation(None, det_file, gt_file, default_evaluation_params, validate_data,
                                                evaluate_method)
